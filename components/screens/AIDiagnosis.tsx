...
import { 
  ArrowLeft, X, Search, ScanLine, Check, Info, 
  Activity, Image as ImageIcon, RotateCcw, BookOpen, Sparkles 
...
export function AIDiagnosis({ onNavigate }: { onNavigate: (screen: string) => void }) {
  const [step, setStep] = useState("entry"); // "entry" | "scan" | "analyzing" | "identified" | "health"
  ...
  const handleSave = () => {
    setSaved(true);
    // TODO: Store나 백엔드에 기록 저장 로직
    setTimeout(() => {
      onNavigate("growth-diary");
    }, 1500);
  };
  ...
  const renderHeader = () => (
    <View style={[styles.appBar, step === "scan" && { backgroundColor: '#000', borderBottomWidth: 0 }]}>
      <TouchableOpacity 
        ...
        onPress={() => {
          if (step === "entry") onNavigate("home");
          else if (step === "health") setStep("identified");
          else setStep("entry");
        }}
      >
  ...
  return (
    <SafeAreaView style={[styles.safeArea, step === "scan" && { backgroundColor: '#000' }]}>
      <StatusBar barStyle={step === "scan" ? "light-content" : "dark-content"} backgroundColor={step === "scan" ? "#000" : "#ffffff"} />
      {renderHeader()}
      ...
      <View style={styles.container}>
        ...
        {/* STEP 2: Scan (Camera Placeholder) */}
        {step === "scan" && (
            ...
            <View style={styles.resultCard}>
              ...
              <View style={styles.infoRow}>
                <View style={styles.infoItem}>
                  ...
                  <Text style={styles.infoValue}>쉬움</Text>
                </View>
                <View style={styles.infoDivider} />
                <View style={styles.infoItem}>
                  <Text style={styles.infoLabel}>빛 요구량</Text>
                  <Text style={styles.infoValue}>반양지</Text>
                </View>
              </View>
          ...
          <ScrollView style={styles.content} contentContainerStyle={styles.scrollContent}>
            <View style={styles.healthHeader}>
              ...
              <Text style={styles.healthDesc}>
                촬영된 몬스테라는 현재 아주 건강한 상태입니다. 새 잎이 돋아날 준비를 하고 있네요!
              </Text>
            </View>
            ...
            <View style={styles.checklist}>
              ...
              <View style={styles.checkItem}>
                ...
                <Text style={styles.checkText}>잎이 선명한 초록색이며 변색이 없어요</Text>
              </View>
              <View style={styles.checkItem}>
                <Check color="#7CCB8A" size={20} style={{ marginRight: 8 }} />
                <Text style={styles.checkText}>해충이나 질병의 징후가 보이지 않아요</Text>
              </View>
              <View style={styles.checkItem}>
                <Check color="#7CCB8A" size={20} style={{ marginRight: 8 }} />
                <Text style={styles.checkText}>현재 물주기와 조명 관리를 유지하세요</Text>
              </View>
            </View>
            ...
            <View style={styles.tipBox}>
              <Text style={styles.tipText}>
                ...
                정기적인 모니터링은 문제를 조기에 발견하는 데 도움이 됩니다. 매주 식물의 변화를 확인하세요.
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  recentList: { gap: 12 },
  recentItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#ffffff', padding: 12, borderRadius: 16, borderWidth: 1, borderColor: '#F3F4F6' },
