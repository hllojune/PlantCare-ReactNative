...
const PlantCard: React.FC<PlantCardProps> = ({ name, species, image, statusText, onPress }) => (
  <TouchableOpacity style={styles.plantCard} onPress={onPress}>
    ...
    <View style={styles.plantInfo}>
      ...
      <Text style={styles.plantStatus}>{statusText}</Text>
    </View>
  </TouchableOpacity>
);
...
export function Home({ navigation }: HomeScreenProps) {
  // React Navigation을 사용한다고 가정하여 navigation prop을 받습니다.
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
        ...
        <View style={styles.appBar}>
          ...
          <View style={styles.appBarRight}>
            ...
            >
              <Image 
                source={{ uri: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=120&h=120&fit=crop&crop=faces" }} 
            ...
            <View style={styles.plantList}>
              {mockPlants.map((plant) => (
                <PlantCard 
                  key={plant.id} 
                  {...plant} 
                  onPress={() => onNavigate("PlantDetail", { plantId: plant.id })} 
                />
              ))}
...
const styles = StyleSheet.create({
  ...
  contentContainer: {
    paddingBottom: 100, // Bottom Navigation 영역 확보
  },
  appBar: {
    flexDirection: 'row',
